...
public class DrawPlaneEnemy {
    public void drawPlaneEnemy(float x, float y ) {
    ...
    private void drawPlaneHead(float xPoint, float yPoint) {
        ...
        float cy =  yPoint + 60; // Tọa độ y của tâm
    ...
    private void drawPilot(float xPoint, float yPoint) {
        ...
        float cx = xPoint ; // Tọa độ x của tâm hình elip
        float cy =yPoint+45; // Tọa độ y của phần thân (đưa phần thân gần đầu hơn)
    ...
    private void drawPlaneWings(float x, float y) { // 2 cánh bên ngoài
        float shiftXLeft = x+ 10f;  // Đẩy cánh trái sang phải một chút
        float shiftXRight = x + -10f; // Đẩy cánh phải sang trái một chút
...
// Nửa trên (màu trắng)
    ...
    private void drawPlaneWings4(float x, float y) {//2 cánh bên ngoài
        float shiftXLeft =x + 10f; // Đẩy cánh trái sang phải một chút
        float shiftXRight =x + -10f; // Đẩy cánh phải sang trái một chút
        float shiftY =y + -90f; // Dịch chuyển cả hai cánh xuống dưới một chút

        // Cánh trái
        glBegin(GL_TRIANGLES);
        glColor3f(1f, 1f, 1f); // Màu xám cho phần thân
        glVertex2f(-50f + shiftXLeft, 20f + shiftY); // Dịch chuyển cánh trái sang phải
        glVertex2f(-10f + shiftXLeft, 45f + shiftY); // Dịch chuyển cánh trái sang phải
        glVertex2f(-10f + shiftXLeft, 35f + shiftY);  // Dịch chuyển cánh trái sang phải
        glEnd();

        // Cánh phải
        glBegin(GL_TRIANGLES);
        glColor3f(1f, 1f, 1f); // Màu xám cho phần thân
        glVertex2f(50f + shiftXRight, 20f + shiftY); // Dịch chuyển cánh trái sang phải
        glVertex2f(10f + shiftXRight, 45f + shiftY); // Dịch chuyển cánh trái sang phải
        glVertex2f(10f + shiftXRight, 35f + shiftY);  // Dịch chuyển cánh trái sang phải
        glEnd();
    }
